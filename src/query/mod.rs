...
pub async fn get_bssid() -> anyhow::Result<Vec<BssidPayload>> {
    ...
    let output = Command::new("nmcli device wifi").output().await?;

    let result: Vec<BssidPayload> = std::str::from_utf8(&output.stdout)?
        .to_string()
        ...
        .map(|(_, line)| {
            ...
            BssidPayload::new(ssid, bssid, rssi, channel, security)
        })
        .collect();
